...
class DQN(object):
    ...
    def step(self, observations, agent_indexs, explore=False):
        """
        ...
        actionList=[]
        selected_obs = []
        for agent_i, a, obs, agent_index in zip(range(self.nagents),self.agents,observations, agent_indexs):
            pol_in = [observations[index] for index in agent_index]
            pol_in.append(observations[agent_i])
            pol_in = torch.cat((*pol_in,), dim=1)
            action=a.step(pol_in)
            selected_obs.append(pol_in.squeeze(0).numpy())
            actionList.append(action)
        return np.array([selected_obs]), actionList
    ...
    @classmethod
    def init_from_env(cls, agent_num=10, agent_alg="DQN", num_in_pol=533*5, num_out_pol=2, discrete_action=True,
                      gamma=0.95, tau=0.01, lr=0.01, hidden_dim=128):
        """
